...
func (peer *Peer) Update(announce *AnnounceRequest) (uint64, uint64) {
	...
	if ul_diff > 0 || dl_diff > 0 {
		log.WithFields(log.Fields{
			...
			"speed_up":     fmt.Sprintf("s%/s", util.Bytes(peer.SpeedUP)),
			"speed_dn":     fmt.Sprintf("s%/s", util.Bytes(peer.SpeedDN)),
			"speed_up_max": fmt.Sprintf("s%/s", util.Bytes(peer.SpeedUPMax)),
			"speed_dn_max": fmt.Sprintf("s%/s", util.Bytes(peer.SpeedDNMax)),
